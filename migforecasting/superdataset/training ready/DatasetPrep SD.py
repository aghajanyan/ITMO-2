...
def normbymax(trainset):
    ...
    tmpp.to_csv("fornorm 24-f 3Ysum.csv", index=False)
...
for i in range(len(rawdata) - ny):
    if rawdata.iloc[i, 0] == rawdata.iloc[i + ny, 0]:
        if rawdata.iloc[i, 2] + ny == rawdata.iloc[i + ny, 2]:  # прогноз только на год вперед
            rawdata.iloc[i, 3] = rawdata.iloc[i + ny, 3] + rawdata.iloc[i + ny - 2, 3] + rawdata.iloc[i + ny - 1, 3]  # сдвигаем inflow / saldo / reg
...
examples.to_csv("superdataset-24-f 3Ysum.csv", index=False)
